  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <div
          onClick={onClick}
          className={`inline-flex items-center gap-3 px-4 py-2.5 rounded-xl bg-gradient-to-r ${updateStyle.bg} border ${updateStyle.border} cursor-pointer group transition-all min-w-[180px]`}
        >
          <div className="relative flex-shrink-0">
            <CalendarIcon className={`w-4 h-4 ${updateStyle.iconColor} transition-colors`} />
            {variant === 'default' && (relativeTime.includes('min') || relativeTime.includes('h')) && (
              <motion.div
                className={`absolute -top-0.5 -right-0.5 w-2 h-2 ${updateStyle.dotColor} rounded-full shadow-sm`}
          ...
          <div className="flex flex-col items-start gap-0.5 flex-1">
            <div className="text-sm font-semibold text-gray-900 dark:text-gray-100 transition-colors whitespace-nowrap">
              {relativeTime}
            </div>
            <div className="text-xs text-gray-500 dark:text-gray-400 transition-colors whitespace-nowrap">
              {fullDate}
            </div>
          </div>
        </div>
