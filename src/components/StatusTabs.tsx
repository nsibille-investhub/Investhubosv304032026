...
type PrimaryStatus = 'need_review' | 'reviewed' | 'all';
type SecondaryStatus = 'rejected' | 'archived' | 'deleted' | 'flagged';
type StatusType = PrimaryStatus | SecondaryStatus;
...
export function StatusTabs({ data, activeStatus, onStatusChange }: StatusTabsProps) {
  
  const calculateKPIs = (filteredData: any[]): StatusKPIs => {
    const total = filteredData.length;
    const withExposure = filteredData.filter(e => e.exposure).length;
    const highRisk = filteredData.filter(e => e.riskLevel === 'High').length;
    const monitoringActive = filteredData.filter(e => e.monitoring).length;
    ...
    return {
      ...
      highRisk,
      monitoringActive,
      exposurePercent: total > 0 ? Math.round((withExposure / total) * 100) : 0,
      highRiskPercent: total > 0 ? Math.round((highRisk / total) * 100) : 0,
    };
  };

  // Filtrer les données selon le statut
  const getFilteredData = (status: StatusType) => {
    switch (status) {
      case 'need_review':
        return data.filter(e => e.status === 'Pending' || e.status === 'New Hit');
      case 'reviewed':
        return data.filter(e => e.status === 'Clear' || e.status === 'True Hit');
      case 'rejected':
        return data.filter(e => e.secondaryStatus === 'rejected');
      case 'archived':
        return data.filter(e => e.secondaryStatus === 'archived');
      case 'deleted':
        return data.filter(e => e.secondaryStatus === 'deleted');
      case 'flagged':
        return data.filter(e => e.secondaryStatus === 'flagged');
      case 'all':
      default:
        return data;
    }
  ...
  const needReviewKPIs = calculateKPIs(getFilteredData('need_review'));
  const reviewedKPIs = calculateKPIs(getFilteredData('reviewed'));
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            ...
            <div className={`flex items-baseline gap-1 ${isActive ? textColor : 'text-gray-900'}`}>
              <span className="text-lg font-bold">{kpis.exposurePercent}%</span>
              <span className="text-xs text-gray-400">({kpis.withExposure})</span>
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <div className={`flex items-baseline gap-1 ${isActive ? textColor : 'text-gray-900'}`}>
              <span className="text-lg font-bold">{kpis.highRiskPercent}%</span>
              <span className="text-xs text-gray-400">({kpis.highRisk})</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-3 pt-3 border-t border-gray-200 flex items-center gap-2 text-xs text-gray-500">
          <div className="flex items-center gap-1">
            <div className={`w-1.5 h-1.5 rounded-full ${kpis.monitoringActive > 0 ? 'bg-green-500' : 'bg-gray-300'}`} />
            <span>{kpis.monitoringActive} monitored</span>
          </div>
        </div>
      </motion.button>
    );
